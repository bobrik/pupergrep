colorMaps = {
  ...
  // Source https://gist.github.com/719710
  // Thanks to Micah Elliott
  // https://github.com/MicahElliott
  xterm256: {
...
ConsoleOutput.prototype.toHtml = function() {
  ...
  // Find an escape code, look for bg and fg colors and replace with proper html.
  while ((matches = regex.exec(htmlContent)) != null) {
    ...
    if (this.isXterm256ColorSequence(matches[1])) {
      var codes = matches[1].split(';'),
          colorCode = parseInt(codes[2]);

      if (codes[0] == '38') {
        // Foreground
        fgColor = '#' + colorMaps.xterm256[colorCode]
      } else {
        // Background
        bgColor = '#' + colorMaps.xterm256[colorCode]
      }
    } else {
      for (var i = 1; i < matches.length; i++) {
        var code = parseInt(matches[i]);

        if (colorMaps.foreground[code]) {
          fgColor = colorMaps.foreground[code];
        } else if (colorMaps.background[code]) {
          bgColor = colorMaps.background[code];
        }
      }
    }
    ...
    if (style.length > 0) {
      htmlContent = htmlContent.replace(new RegExp("\\x1b\\[" + matches[1] + 'm', 'g'), '<span style="' + style.join('; ') + '">');
    } else {
      htmlContent = htmlContent.replace(new RegExp("\\x1b\\[" + matches[1] + 'm', 'g'), '<span>');
    }
...
jQuery.fn.colorizeConsoleOutput = function() {
  return this.each(function () {
    ...
    $(this).html(consoleOutput.toHtml());
  });
};
