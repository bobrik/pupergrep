(function(module) {
    ...
    PuperGrep.prototype.add = function(logpath, logType, processor) {
        var self    = this,
            manager = self.getLogReaderManager();

        logname = path.basename(logpath);
        manager.addLog(logname, logpath, function(error) {
            if (error) {
                console.log("Error adding "+logname+" log", error);
                return;
            }
        });
        ...
        switch (logType) {
            case "html":
                manager.setLogType(logname, "html");
                break;
            case "ansi":
            default:
                manager.setLogType(logname, "ansi");
                break;
        }
        ...
        manager.getLogReader(logname, function(error, reader) {
            if (error) {
                console.log(error);
                return;
            }
